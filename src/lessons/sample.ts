...
// The correct final component the editor types toward and the preview runs.
...
// A classic bug: calling the setter directly (no arrow) runs it on every render.
  ...
  scenes: [
    ...
    },
    {
      ...
      action: {
        ...
        file: '/src/App.jsx',
        type: BODY,
        callouts: [{ line: 4, text: 'value + setter', kind: 'tip' }],
      },
    },
    {
      id: 's4',
      chapter: 'A common bug',
      focus: 'editor',
    ...
    },
    {
      ...
      chapter: 'Try it',
      focus: 'preview',
      narration: 'Click the button — `setCount` runs and React re-renders. Watch the count rise.',
      say: 'Click the button. setCount runs and React re-renders. Watch the count rise.',
      autoAdvanceMs: 5500,
      action: { tool: 'preview', steps: [{ cmd: 'click', selector: 'button' }, { cmd: 'click', selector: 'button' }] },
    },
    {
      id: 's8',
      chapter: 'Devtools',
      focus: 'preview',
      narration: 'Open the **network** tab — the `fetch` to `/todos/1` returns the tip we render.',
      say: 'Open the network tab. The fetch to todos one returns the tip we render.',
      autoAdvanceMs: 5500,
