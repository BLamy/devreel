...
function almostnodeHosting() {
  ...
  return {
    ...
    generateBundle() {
      ...
      this.emitFile({ type: 'asset', fileName: '__sw__.js', source: fs.readFileSync(swSource, 'utf8') })
    },
  }
}
...
export default defineConfig({
  plugins: [
    ...
    nodePolyfills({
      include: [
        'path', 'util', 'events', 'buffer', 'url', 'string_decoder',
        'querystring', 'crypto', 'assert', 'stream', 'http', 'https',
        'zlib', 'tty', 'os',
      ],
      ...
      plugins: [
        {
          ...
          setup(build: { onResolve: (opts: { filter: RegExp }, cb: () => { path: string }) => void }) {
            build.onResolve({ filter: /^(node:)?stream\/promises$/ }, () => ({ path: streamPromisesStub }))
          },
        },
      ],
    },
